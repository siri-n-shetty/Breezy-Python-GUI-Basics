...
class NumberFieldDemo(EasyFrame):
    ...
    def __init__(self):
        ...
        self.inputField = self.addIntegerField(value = 0, row = 0, column = 1, width = 15)

        # Label and field for the output
        self.addLabel(text = "Square root", row = 1, column = 0)
        self.outputField = self.addFloatField(value = 0.0, row = 1, column = 1, width = 15, 
                                              precision = 2, state = "readonly")
        #precision rounds off the output field to 2 digits
    ...
    def computeSqrt(self):
        ...
        """Handles input errors by displaying a message"""
        try:
